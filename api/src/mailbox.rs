...
impl SocToCaliptra {
    pub fn execute_req<R: crate::mailbox::Request>(
        &mut self,
        mut req: R,
        ...
        self.mailbox_execute(
            R::ID.into(),
            req.as_bytes(),
            response_bytes.data.as_bytes_mut(),
            delay_one_cycle,
        )?;

        if response_bytes.data.len() < R::Resp::MIN_SIZE
            || response_bytes.data.len() > core::mem::size_of::<R::Resp>()
        {
            return Err(MailboxError::MailboxUnexpectedResponseLen {
                expected_min: R::Resp::MIN_SIZE as u32,
                expected_max: core::mem::size_of::<R::Resp>() as u32,
                actual: response_bytes.data.len() as u32,
            });
        }

        let response_header =
            MailboxRespHeader::read_from_prefix(response_bytes.data.as_bytes()).unwrap();
        let actual_checksum = calc_checksum(0, &response_bytes.data[4..]);
        if actual_checksum != response_header.chksum {
            return Err(MailboxError::MailboxRespInvalidChecksum {
                expected: response_header.chksum,
                actual: actual_checksum,
            });
        }
        if response_header.fips_status != MailboxRespHeader::FIPS_STATUS_APPROVED {
            return Err(MailboxError::MailboxRespInvalidFipsStatus(
    ...
    fn finish_mailbox_execute(
        ...
        if buf.len() < dlen as usize {
            return Err(MailboxError::MailboxResponseTooLarge);
        }
