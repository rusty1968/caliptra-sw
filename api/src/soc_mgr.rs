...
pub trait SocManager {
    ...
    fn send_firmware_load_req(&mut self, firmware: &[u8]) -> Result<(), CaliptraApiError> {
        let response = self.mailbox_exec(crate::mailbox::CommandId::FIRMWARE_LOAD.into(), firmware, &mut [])?;
        if response.is_some() {
